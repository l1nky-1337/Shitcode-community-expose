namespace MouseController
{
	...
	static BOOLEAN WINAPI Init()
	{
		...
		LPVOID NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(_XOR_("win32u")), _XOR_("NtUserSendInput"));
		if (!NtUserSendInput_Addr)
		{
			NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(_XOR_("user32")), _XOR_("NtUserSendInput"));
			if (!NtUserSendInput_Addr)
			{
				// Windows 7 or lower detected
				NtUserSendInput_Addr = GetProcAddress(GetModuleHandle(_XOR_("user32")), _XOR_("SendInput"));
				if (!NtUserSendInput_Addr)
					return FALSE;
			}
		}
		...
		// windows 8.1 / windows 10
		LPVOID NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(_XOR_("win32u")), _XOR_("NtUserGetAsyncKeyState"));
		if (!NtUserGetAsyncKeyState_Addr)
		{
			NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(_XOR_("user32")), _XOR_("NtUserGetAsyncKeyState"));
			if (!NtUserGetAsyncKeyState_Addr)
			{
				// Windows 7 or lower detected
				NtUserGetAsyncKeyState_Addr = GetProcAddress(GetModuleHandle(_XOR_("user32")), _XOR_("GetAsyncKeyState"));
				if (!NtUserGetAsyncKeyState_Addr)
					return FALSE;
			}
		}
