...
#define HJWND_PROGRAM L"mmc.exe"
...
#define TRANSPARENCY_COLOR RGB(0, 254, 0)
    ...
    inline int amain() {
        ...
        if (!hwnd) {
            cout << _XOR_("Window HiJacking failed (use debugger to investigate why)") << endl;
            return EXIT_FAILURE;
        }
    ...
    inline HWND HiJackNotepadWindow() {
        ...
        vector<DWORD> existingNotepads = GetPIDs(_XOR_(L"mmc.exe"));
        if (!existingNotepads.empty()) {
            for (int i(0); i < existingNotepads.size(); ++i) {
        ...
        system(_XOR_("start mmc")); // Start notepad, and not as child process, so easy :)

        // Finding notepad's window (we could just use FindWindow but then it would be OS language dependent)
        vector<DWORD> notepads = GetPIDs(_XOR_(L"mmc.exe"));
        ...
        DriverVar::width = GetSystemMetrics(SM_CXSCREEN);
        DriverVar::height = GetSystemMetrics(SM_CYSCREEN);
        return hwnd;
    }
